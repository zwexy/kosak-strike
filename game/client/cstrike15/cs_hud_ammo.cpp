...
void CHudAmmo::OnThink()
{
    ...
    if ( !player )
        return;
    if( player->GetObserverMode() && player->GetObserverTarget())
    {
        player = ToCSPlayer( player->GetObserverTarget() );
		if ( !player )
			return;
    }
	...
	else
	{
        ...
        ammo2 = wpn->GetReserveAmmoCount( AMMO_POSITION_PRIMARY );//player->GetAmmoCount(wpn->GetPrimaryAmmoType());
    }
    ...
    m_Ammo1Color = (float)ammo1 / (float)wpn->GetMaxClip1() <= 0.2 ? COLOR_RED : GetFgColor();
    m_Ammo2Color = ammo2 + 1 <= wpn->GetMaxClip1() ? COLOR_RED : GetFgColor();
...
void CHudAmmo::SetAmmo(int ammo, bool playAnimation)
{
	if (ammo != m_iAmmo)
	{
        if (ammo == 0)
		{
            GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("PrimaryAmmoEmpty");
		}
		else if (ammo < m_iAmmo)
		{
			// ammo has decreased
            GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("PrimaryAmmoDecrement");
		}
		else
		{
			// ammunition has increased
            GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("PrimaryAmmoIncrement");
		}
...
void CHudAmmo::SetAmmo2(int ammo2, bool playAnimation)
{
	if (ammo2 != m_iAmmo2)
	{
		if (ammo2 == 0)
		{
            GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("SecondaryAmmoEmpty");
		}
		else if (ammo2 < m_iAmmo2)
		{
			// ammo has decreased
            GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("SecondaryAmmoDecrement");
		}
		else
		{
			// ammunition has increased
            GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("SecondaryAmmoIncrement");
		}
...
void CHudAmmo::Paint( void )
{
	...
	x += ( 2.5 - digits ) * charWidth;

	// draw primary ammo
    vgui::surface()->DrawSetTextColor(m_Ammo1Color);
